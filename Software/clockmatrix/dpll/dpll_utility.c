...
static void usage(const char *prog)
{
    fprintf(stderr,
        ...
        "  --read <addr>           Read 8-bit value from 16-bit DPLL register (hex or dec).\n"
        "  --write <addr> <data>   Write 8-bit value to 16-bit DPLL register.\n"
        "  --flash-hex <hex>       Program EEPROM via DPLL's I2C master using Intel HEX file.\n"
        "  --tcs-apply <tcs>       Apply a Timing Commander .tcs file live (register writes).\n"
        "  --prog-file <txt>       Apply a Timing Commander programming .txt file\n"
        "                          (Offset/Size/Data lines via dpll_apply_program_file).\n"
        "\n"
        "Monitor/daemon helper commands (script-friendly output):\n"
        "  get_state <chan>\n"
        "      Print channel state as a single token (LOCKED|LOCKACQ|LOCKREC|FREERUN|HOLDOVER|...).\n"
        "  get_statechg_sticky <chan>\n"
        "      Print 0/1 indicating whether the channel state changed since last clear.\n"
        "  clear_statechg_sticky <chan>\n"
        "      Clear the channel state-change sticky bit.\n"
        "  set_oper_state <chan> <NORMAL|FREERUN|HOLDOVER>\n"
        "      Force channel operating state (used to retrigger lock acquisition).\n"
        "  get_phase <chan>\n"
        "      Print signed phase measurement for <chan> in seconds (single float).\n"
        "\n"
        "High-level DPLL control (used by boot-time Python config):\n"
        "  set-input-freq <input> <freq_hz>   (or --set-input-freq)\n"
        "      Configure logical DPLL input index (1..4) nominal frequency in Hz.\n"
        "  set-input-enable <input> <enable|disable>   (or --set-input-enable)\n"
        "      Enable/disable a logical DPLL input (1..4).\n"
        "  set-chan-input <chan> <input> <priority> <enable|disable>   (or --set-chan-input)\n"
        ...
        "      Configure logical outputs 3 and 4 simultaneously (they share a source/divider).\n"
        "      Each <freq*_hz> may be integer or floating-point (e.g. 10e6, 10.000001e6).\n"
        "\n"
        "Connection options:\n"
        "  --spidev <path>         SPI node (e.g. /dev/spidev2.1). Overrides bus/cs.\n"
        "  --busnum <n>            SPI bus number -> /dev/spidev<n>.<csnum> if --spidev not used.\n"
        "  --csnum <m>             SPI chip-select -> /dev/spidev<busnum>.<m> if --spidev not used.\n"
        "  --hz <freq>             SPI clock (Hz), default 1000000.\n"
        "  --mode <0..3>           SPI mode, default 0.\n"
        "\n"
        "Debug options:\n"
        "  --tcs-debug             Make TCS/TXT parsers verbose (print parsed/written registers).\n"
        "\n"
        "Examples:\n"
        "  %s --read  0xC024\n"
        ...
        "  %s --prog-file Test_FromScratch_Programming_11-26-2025.txt --tcs-debug\n"
...
{
    uint32_t tmp;
    if (parse_u32(s, &tmp) != 0 || tmp > 0xFFFF) return -1;
    *out = (uint16_t)tmp;
    return 0;
}
...
{
    uint32_t tmp;
    if (parse_u32(s, &tmp) != 0 || tmp > 0xFF) return -1;
    *out = (uint8_t)tmp;
    return 0;
}
...
static void flash_progress_cb(size_t written, size_t total, void *user)
{
    ...
    if (secs_since(now, ctx->last_print) < 0.10 && written != total) return;
    ctx->last_print = now;
    ...
    if (total > 0) {
        double eta = (rate > 0)
                     ? ((double)total - (double)written) / rate
                     : 0.0;
        fmt_hms(eta, tbuf);
...
{
   fprintf(stderr,
            "dpll_cmd_set_input_freq: input=%u freq=%.6f Hz\n",
            input_idx, freq_hz);
    ...
    rc = cm_string_write_bytes(&g_cm_bus,
                               "Input",
                               input_idx,
                               "INPUT_IN_FREQ_M_0_7",
                               buf_M,
    ...
    if (rc) {
        fprintf(stderr,
                "Failed to write M for Input[%u], rc=%d\n",
                input_idx, rc);
        return rc;
    }

    /* Pack N as 2 bytes LSB-first: N[0..7], 8..15, stored in N_reg. */
    uint16_t N = N_reg;  // N_reg is already encoded (0 means 1), but if N!=1 you just write it.
    uint8_t buf_N[2];
    ...
    if (rc) {
        fprintf(stderr,
                "Failed to write N for Input[%u], rc=%d\n",
                input_idx, rc);
        return rc;
    }
...
{
    fprintf(stderr,
            "dpll_cmd_set_input_enable: input=%u state=%s \n",
            input_idx, enable ? "enable" : "disable");
 ...
 */
static int dpll_cmd_set_chan_input(uint8_t chan,
                                   uint8_t input_idx,
                                   uint8_t priority,
                                   int enable)
{
    fprintf(stderr,
            ...
            chan, input_idx, priority, enable ? "enable" : "disable");
 ...
 * For your board, output 2 is usually 1PPS, dont allow user to change; outputs 3 & 4 may be arbitrary freq.
 */
static int dpll_cmd_set_output_freq(double freq3_hz, double freq4_hz)
{
    ...
    int rc = dpll_compute_output_mndiv(freq3_hz, freq4_hz,
                                       &M, &N_reg,
                                       &D3, &D4,
                                       &fdco,
                                       &out3_actual, &out4_actual,
                                       &out3_err, &out4_err);
    if (rc != 0) {
    ...
    fprintf(stderr,
            ...
            (unsigned long long)M, (unsigned)N_reg,
            fdco, fdco / 1e6,
            D3, out3_actual, out3_err, e3_ppm,
            D4, out4_actual, out4_err, e4_ppm);
        ...
        cm_string_write_bytes(&g_cm_bus, "DPLL_Ctrl", 6,
                              "FOD_FREQ_M_7_0", m_bytes, 6);
	...
	// don't need trigger, DPLL_Ctrl, every register is trigger register!
        ...
        cm_string_write_bytes(&g_cm_bus, "Output", 10,
                              "OUT_DIV", d3_bytes, 4);
        ...
        cm_string_write_bytes(&g_cm_bus, "Output", 11,
                              "OUT_DIV", d4_bytes, 4);
 ...
 */
static int dpll_ll_get_lock_state(cm_bus_t *bus, unsigned chan, dpll_lock_state_t *out_state);
static int dpll_ll_get_statechg_sticky(cm_bus_t *bus, unsigned chan, uint8_t *out_sticky);
static int dpll_ll_clear_statechg_sticky(cm_bus_t *bus, unsigned chan);
static int dpll_ll_set_oper_state(cm_bus_t *bus, unsigned chan, dpll_oper_state_t state);

static const char *dpll_lock_state_to_str(dpll_lock_state_t st)
{
    switch (st) {
        case DPLL_LOCK_STATE_LOCKED:   return "LOCKED";
        case DPLL_LOCK_STATE_LOCKACQ:  return "LOCKACQ";
        case DPLL_LOCK_STATE_LOCKREC:  return "LOCKREC";
        case DPLL_LOCK_STATE_FREERUN:  return "FREERUN";
        case DPLL_LOCK_STATE_HOLDOVER: return "HOLDOVER";
        case DPLL_LOCK_STATE_DISABLED: return "DISABLED";
...
static int dpll_cmd_set_oper_state(uint8_t chan, const char *state_str)
{
    ...
    if (dpll_parse_oper_state(state_str, &st) != 0) {
        fprintf(stderr, "set_oper_state: bad state '%s' (expected NORMAL|FREERUN|HOLDOVER)\n",
...
{
    int64_t phase_s36 = 0;
    int rc = cm_read_phase_status_s36((const cm_bus_t *)&g_cm_bus, (unsigned)chan, &phase_s36);
    if (rc != 0) {
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        /* Monitor/daemon helper commands (accept with or without leading --) */
                      ...
                      !strcmp(argv[i], "--get-state")) && i+1 < argc) {
            uint32_t tmp;
            ...
            do_get_state = 1;
                      ...
                      !strcmp(argv[i], "--get-statechg-sticky")) && i+1 < argc) {
            uint32_t tmp;
            if (parse_u32(argv[++i], &tmp) != 0 || tmp > 255) {
            ...
            do_get_statechg_sticky = 1;
                      ...
                      !strcmp(argv[i], "--clear-statechg-sticky")) && i+1 < argc) {
            uint32_t tmp;
            if (parse_u32(argv[++i], &tmp) != 0 || tmp > 255) {
            ...
            do_clear_statechg_sticky = 1;
                      ...
                      !strcmp(argv[i], "--set-oper-state")) && i+2 < argc) {
            uint32_t tmp;
            if (parse_u32(argv[++i], &tmp) != 0 || tmp > 255) {
            ...
            do_set_oper_state = 1;
                      ...
                      !strcmp(argv[i], "--get-phase")) && i+1 < argc) {
            uint32_t tmp;
            if (parse_u32(argv[++i], &tmp) != 0 || tmp > 255) {
        ...
        /* High-level commands (accept with or without leading --) --------- */
                     ...
                     !strcmp(argv[i], "--set-input-freq")) && i+2 < argc) {
		...
		fprintf(stderr, "Bad set-input-freq freq_hz (must be > 0, can be float)\n");
		return 1;
	    }
	    hl_freq_hz = ftmp;
	    ...
	    do_set_input_freq = 1;
                     ...
                     !strcmp(argv[i], "--set-input-enable")) && i+2 < argc) {
            uint32_t tmp;
            if (parse_u32(argv[++i], &tmp) != 0 ) {
            ...
            if (!strcmp(st, "enable")) {
                ...
                fprintf(stderr, "Bad set-input-enable state (must be enable|disable)\n");
                return 1;
            }
            do_set_input_enable = 1;

        } else if ( (!strcmp(argv[i], "set-chan-input") ||
                     !strcmp(argv[i], "--set-chan-input")) && i+4 < argc) {
            uint32_t tmp;
            if (parse_u32(argv[++i], &tmp) != 0 || tmp > 255) {
            ...
            do_set_chan_input = 1;
                     ...
                     !strcmp(argv[i], "--set-output-freq")) && i+2 < argc) {
		// specific utility for switchberry, could be adapted later for more generic

	    double f3 = 0.0;
	    double f4 = 0.0;

	    if (parse_double(argv[++i], &f3) != 0 || f3 <= 0.0) {
		fprintf(stderr, "Bad set-output-freq freq3_hz (must be > 0, can be float)\n");
		return 1;
	    }
	    if (parse_double(argv[++i], &f4) != 0 || f4 <= 0.0) {
		fprintf(stderr, "Bad set-output-freq freq4_hz (must be > 0, can be float)\n");
		return 1;
	    }
	    ...
	    do_set_output_freq = 1;
             ...
             !strcmp(argv[i], "--out-phase-adj-get")) && i+1 < argc) {
    ...
    do_out_phase_adj_get = 1;
             ...
             !strcmp(argv[i], "--out-phase-adj-set")) && i+2 < argc) {
    if (parse_u32(argv[++i], &hl_out_idx) != 0) {
    ...
    do_out_phase_adj_set = 1;
             ...
             !strcmp(argv[i], "--wr-freq-get")) && i+1 < argc) {
    ...
    do_wr_freq_get = 1;
             ...
             !strcmp(argv[i], "--wr-freq-set-word")) && i+2 < argc) {
    if (parse_u32(argv[++i], &hl_dpll_idx) != 0) {
    ...
    do_wr_freq_set_word = 1;
             ...
             !strcmp(argv[i], "--wr-freq-set-ppb")) && i+2 < argc) {
    if (parse_u32(argv[++i], &hl_dpll_idx) != 0) {
    ...
    int action_count =
        do_read + do_write + do_flash + do_tcs_apply +
        do_get_state + do_get_statechg_sticky + do_clear_statechg_sticky +
        do_set_oper_state + do_get_phase +
        do_set_input_freq + do_set_input_enable +
        do_set_chan_input + do_set_output_freq + do_set_out2_dest + do_prog_file +
        do_out_phase_adj_get + do_out_phase_adj_set +
        do_wr_freq_get + do_wr_freq_set_word + do_wr_freq_set_ppb;

    if (action_count != 1) {
        fprintf(stderr,
                "Specify exactly one action: "
                "--read, --write, --flash-hex, --tcs-apply,\n"
                "  or monitor helpers get_state, get_statechg_sticky, clear_statechg_sticky, set_oper_state, get_phase,\n"
    ...
    if (!spidev_overridden && (have_busnum || have_csnum)) {
        snprintf(spidev_path, sizeof(spidev_path),
                 "/dev/spidev%u.%u", busnum, csnum);
    }
	...
	*/
    ...
    if (do_read) {
        ...
        dpll_result_t r = dpll_eeprom_flash_hex(spi_fd,
                                                hex_path,
                                                flash_progress_cb,
                                                &ctx);
        if (r != DPLL_OK) {
        ...
        if (dpll_cmd_set_chan_input(hl_chan,
                                    hl_input_for_chan,
                                    hl_priority,
                                    hl_enable_flag) != 0) {
    ...
    int32_t adj = 0;
    int rr = cm_read_output_phase_adj_s32(&g_cm_bus, (unsigned)hl_out_idx, &adj);
    if (rr != 0) {
        fprintf(stderr, "out-phase-adj-get failed (out=%u), rc=%d\n", hl_out_idx, rr);
        rc = 1;
    } else {
        printf("Output[%u].OUT_PHASE_ADJ = %d (0x%08x)\n",
               hl_out_idx, adj, (uint32_t)adj);
    }

} else if (do_out_phase_adj_set) {
    int rr = cm_write_output_phase_adj_s32(&g_cm_bus, (unsigned)hl_out_idx, hl_phase_adj, 1, 0);
    if (rr != 0) {
        ...
        printf("Wrote Output[%u].OUT_PHASE_ADJ = %d (0x%08x)\n",
               hl_out_idx, hl_phase_adj, (uint32_t)hl_phase_adj);
    }

} else if (do_wr_freq_get) {
    int64_t word = 0;
    int rr = cm_read_dpll_wr_freq_s42(&g_cm_bus, (unsigned)hl_dpll_idx, &word, 1);
    if (rr != 0) {
    ...
    int rr = cm_write_dpll_wr_freq_s42(&g_cm_bus, (unsigned)hl_dpll_idx, hl_wr_word_s42, 1, 0);
    if (rr != 0) {
    ...
    int rr = cm_write_dpll_wr_freq_s42(&g_cm_bus, (unsigned)hl_dpll_idx, word, 1, 0);
    if (rr != 0) {
        fprintf(stderr, "wr-freq-set-ppb failed (dpll=%u, ppb=%.9f, word=%lld), rc=%d\n",
                hl_dpll_idx, hl_wr_ppb, (long long)word, rr);
        rc = 1;
    } else {
        printf("Wrote DPLL_Freq_Write[%u].DPLL_WR_FREQ ~%.9f ppb (word_s42=%lld)\n",
               hl_dpll_idx, hl_wr_ppb, (long long)word);
    }

	} else if (do_prog_file) {
	    fprintf(stderr, "Applying programming file: %s\n", prog_path);
	    dpll_result_t r = dpll_apply_program_file(spi_fd, prog_path, 1); // always verbose
	    //dpll_result_t r = dpll_apply_program_file(spi_fd, prog_path, tcs_debug ? 1 : 0);
	    if (r != DPLL_OK) {
...
static int dpll_ll_get_lock_state(cm_bus_t *bus, unsigned chan, dpll_lock_state_t *out_state)
{
    ...
    uint8_t buf = 0;
    int rc = cm_string_field_read8(bus, "Status", 0, reg_name, "DPLL_STATE", &buf);
...
static int dpll_ll_get_statechg_sticky(cm_bus_t *bus, unsigned chan, uint8_t *out_sticky)
{
    ...
    uint8_t buf = 0;
    int rc = cm_string_field_read8(bus, "Status", 0, reg_name, "LOCK_STATE_CHANGE_STICKY", &buf);
    if (rc != 0) return rc;
    *out_sticky = buf;
    return rc;
}
...
static int dpll_ll_set_oper_state(cm_bus_t *bus, unsigned chan, dpll_oper_state_t state)
{
    ...
    int rc = cm_string_field_write8(bus, "DPLL_Config", chan, "DPLL_MODE", "STATE_MODE", (uint8_t) state);
    return rc;
}
